    ...
    return (
        <div className="flex w-full mx-auto mt-8 p-6 rounded-lg">
            <div className="flex flex-col w-3/12 space-y-4 border-r border-[var(--boxBorder)] py-2">
                {data && data?.map((avail, index: number) => (
            ...
            <div className="table-auto w-full flex flex-col">
                <table>
                    <tbody className="w-1/2">
                        <InfoDisplayComponent label="Start Time" value={data && data[tab]?.startTime} />
                        <InfoDisplayComponent label="End Time" value={data && data[tab]?.endTime} />
                        <InfoDisplayComponent label="Duration" value={data && data[tab]?.duration} />
                        <InfoDisplayComponent label="Service Modes" value={data && data[tab]?.modes.map((item: string) => item + " ")} />
                    </tbody>
                </table>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 px-2">
                    {data && data[tab]?.slots.map((slot: Slot) => (
                        <div
                            key={slot._id}
