...
const PaymentSelection: React.FC<PaymentSelectionProps> = ({ paymentSelectionRef }) => {
    ...
    const { paymentSelectionOpen, planId } = useSelector((state: RootState) => state.provider);
    ...
    return paymentSelectionOpen && (
        <div className="fixed inset-0 flex justify-center items-center bg-black/70 z-50">
            {!paymentLoading ? (
                <div className="w-3/12 rounded-lg shadow-lg border p-4" ref={paymentSelectionRef}>
                    ...
                    <div className="py-6">
                        ...
                        <SelectFiledWithLabel
                            ...
                            required
                        />
                        <button className="w-full flex items-center justify-center p-3 rounded-md shadow cursor-pointer hover:bg-gray-100" onClick={makeStripePayment}>
